...
int __get_inode_from_dir(fs_ext2_ctx_t *ctx, const inode_t *dir, char *name,
  ...
  for (int dptri = 0; dptri < 12; dptri++) {
    ...
    while (((void *)dir_entry - (void *)dir_entry_buf) < ctx->block_sz) {
      ...
      // printf("\nNode (%d == %d): ", name_length, cur_dentry_name_length);
...
// ret must have a size for which % block_sz == 0
int read_from_inode_full(inode_t i, void *ret) {
  return __read_from_inode(&FS_GLOBAL_CTX, i,
                           ceild(i.lsize, FS_GLOBAL_CTX.block_sz), ret);
}
...
void init_fs(uint32_t part_off) {
  ...
  // inode_t test_inode;
  // int ret = traverse("hello", NULL, &test_inode);
  // if (!ret) {
  //   printf("Got inode hello!\n");
  //   char buf[8192];
  //   int i = 0;
  //   __read_from_inode(&FS_GLOBAL_CTX, test_inode, 1, buf);
  //   while (buf[i]) {
  //     printf("%c", buf[i]);
  //     i++;
  //   }
