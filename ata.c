...
static uint8_t poll() {
  ...
  for (int i = 0; i < 4; i++) {
    stat = inb(IO_STATUS);
    if (!get_bit(&stat, 7) &&
        get_bit(&stat, 5)) // DF/ERR could still be set accidentally for the
                           // first four reads
      return stat;
  }
  for (;;) {
    stat = inb(IO_STATUS);
    if (get_bit(&stat, 0) | get_bit(&stat, 3) |
        (!get_bit(&stat, 7) && get_bit(&stat, 5)))
      return stat;
  }
}
...
static void drv_select(bool select_master) {
...
// addr is starting address in bytes (must be aligned to BYTES_PER_SECTOR for
// now) size is the size of sectors in bytes
int read_ata(bool read_master, uint32_t addr, uint32_t size, uint16_t *buf) {
  if (!read_master) {
    printf("read: TODO!\n");
    return -1;
  }
  read_internal(addr / BYTES_PER_SECTOR,
                size / BYTES_PER_SECTOR + (size % BYTES_PER_SECTOR > 0 ? 1 : 0),
                buf);
  return 0;
}
...
uint32_t get_lba_cnt(uint16_t identify_data[256]) {
  ...
  return ((uint32_t)identify_data[lba_cnt_off]) |
         ((uint32_t)identify_data[lba_cnt_off + 1] << 16);
}
